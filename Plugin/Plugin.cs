    ...
    public class LogicProxyDefaultImpl : ILogicCharacterProxy
    {
        ...
        public IEnumerator EnchanceEquip(Coroutine coroutine, LogicCharacterProxy charProxy, EnchanceEquipInMessage msg)
        {
            ...
            var errorCodes = proxy.Character.EnchanceEquip(msg.Request.BagType, msg.Request.BagIndex,
                msg.Request.Blessing, msg.Request.UpRate, ref nextLevel);
        ...
        {
            var sceneFubenMsg = LogicServer.Instance.SceneAgent.SSGetFubenStoreItems(charProxy.CharacterId, shopType);
            yield return sceneFubenMsg.SendAndWaitUntilDone(coroutine);

            if (sceneFubenMsg.ErrorCode == (int)ErrorCodes.OK)
                itemList.AddRange(sceneFubenMsg.Response.items);
        }
        ...
        public IEnumerator UseGiftCode(Coroutine coroutine, LogicCharacterProxy _this, UseGiftCodeInMessage msg)
        {
            ...
            var t = LogicServerControl.GiftCodeDb.UpdateStateAsync(code);
            switch (t.Result)
            {
                case eDbReturn.Success:
                    {
                                                ...
                                                ApplyFirstChargeItemInMessage msg)
        {
            ...
            foreach (var dicData in LogicServer.Instance.ServerControl.FirstChargeItemDict)
            {
                ...
                itemList.isCharged = prox.Character.GetExData((int)eExdataDefine.e625) >= itemList.diamond ? 1 : 0;
                                                   ...
                                                   ApplyGetFirstChargeItemInMessage msg)
        {
            ...
            // 充值累计金额达到该档位  且  没有领取该档位奖励   
            if (prox.Character.GetExData((int)eExdataDefine.e625) >= tbFirsrtCharge.diamond
        ...
        public IEnumerator ApplyMailInfo(Coroutine coroutine, LogicCharacterProxy charProxy, ApplyMailInfoInMessage msg)
        {
            ...
            else
            {
                ...
                if (Mail.Reward.Count < 1 || Mail.State == (int)MailStateType.Receive)
                {
                ...
                case 200:
                    {
                        ...
                        var freeCount = proxy.Character.GetExData((int)eExdataDefine.e411);
                        if (freeCount > 0)
                        {
                            proxy.Character.SetExData((int)eExdataDefine.e411, freeCount - 1);
                        }
                        else
                        {
            ...
            ApplyGetTowerRewardInMessage msg)
        {
            ...
            var timesMsg = LogicServer.Instance.ActivityAgent.SSAskMieshiTowerReward(proxy.CharacterId, msg.Request.ServerId, msg.Request.ActivityId, charProxy.CharacterId, msg.Request.Idx);
        ...
        public IEnumerator ApplyPromoteHP(Coroutine coroutine, LogicCharacterProxy charProxy, ApplyPromoteHPInMessage msg)
        {
            ...
            if (proxy.Character.mBag.GetRes(eResourcesType.DiamondRes) < tb.DiamondCost)
            {
                msg.Reply((int)ErrorCodes.DiamondNotEnough);
                yield break;
            }
            ...
            var retMsg = LogicServer.Instance.ActivityAgent.SSApplyPromoteHP(charProxy.CharacterId,
                msg.Request.ServerId, msg.Request.ActivityId, msg.Request.BatteryId, msg.Request.PromoteType, proxy.CharacterId, proxy.Character.GetName());
            ...
            ApplyPromoteSkillInMessage msg)
        {
            ...
            var sceneMsg = LogicServer.Instance.SceneAgent.SSApplyNpcHP(proxy.CharacterId, msg.Request.ServerId, tbMieShi.FuBenID, msg.Request.BatteryGuid);
            yield return sceneMsg.SendAndWaitUntilDone(coroutine);
        ...
        public IEnumerator CSApplyOfflineExpData(Coroutine coroutine, LogicCharacterProxy charProxy, CSApplyOfflineExpDataInMessage msg)
        {
            ...
            // 算出离线时间   
            var lastoffTime = prox.Character.lExdata64.GetTime(Exdata64TimeType.LastOutlineTime);
            ...
            if (offlineRecord.Time != 0 && offlineRecord.Exp != 0)
            {
                ...
                proxy.Character.mBag.AddRes(eResourcesType.ExpRes, (int)addExp, eCreateItemType.Offline);
