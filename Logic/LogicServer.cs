...
namespace Logic
{
    public class LogicServer : ServerBase, IServer
    {
        private static LogicServer _instance;
        ...
        {
            get { return _instance ?? (_instance = new LogicServer()); }
        }

        public LogicServerControl ServerControl { get; private set; }
        public UpdateManager UpdateManager { get; private set; }

        public new void Init(string[] args)
        {
            Logger.Info(@"Logic server initlizing...");
            ...
            UpdateManager.Init("Logic");

            ServerControl = new LogicServerControl();

            if (!int.TryParse(args[1], out mPort))
            {
                Logger.Warn(@"Logic server initlizing Faild! args[1]={0}", args[1]);
                return;
            }
            base.Init(args);
            int nId;
            ...
            {
                Logger.Warn(@"Logic server initlizing Faild! args[0]={0}", args[0]);
                return;
            }
            Id = (uint) nId;
            mAuth = args[2];
            var strDbConfig = args[4];
            DB = new DataManager(strDbConfig, ServerControl, DataCategory.Logic, nId);
        }

        public void Start()
        {
            Logger.Info(@"Logic server starting...");
        ...
        {
            Logger.Info(@"Logic server stopping...");
